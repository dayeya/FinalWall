#
# FinalWall - Networking functions of a reverse proxy.
# Author: Dayeya.
#

from typing import Callable

from finalwall.net import Connection
from finalwall.http_process import SearchContext, search_header, contains_body_seperator
...
async def recv_from_server(server: Connection) -> bytes:
    ...
    content: bytes = await server.recv_until(
        condition=lambda _content, _content_length: len(_content) >= _content_length,
        args=(content_length,)
    )
